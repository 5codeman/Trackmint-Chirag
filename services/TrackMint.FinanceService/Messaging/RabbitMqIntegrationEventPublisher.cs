...
namespace TrackMint.FinanceService.Messaging;
...
{
    private readonly RabbitMqOptions _options = configuration.GetSection("RabbitMq").Get<RabbitMqOptions>() ?? new RabbitMqOptions();
    ...
    public Task PublishAsync(IntegrationEvent integrationEvent, string routingKey, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            logger.LogWarning(
                ex,
                "RabbitMQ publish failed. Business transaction remains committed. EventType={EventType} RoutingKey={RoutingKey}",
                integrationEvent.EventType,
                routingKey);
        }
